...
function getUserProfile() {
  ...
  if (!sheet) {
    sheet = ss.insertSheet(SHEET_NAMES.USERS);
    sheet.getRange('A1:E1').setValues([['Email', 'FirstName', 'LastName', 'Phone', 'StudentID']]);
    return null;
  }
...
function saveUserProfile(profile) {
  ...
  const rowData = [email, profile.firstName, profile.lastName, profile.phone, profile.studentId];
